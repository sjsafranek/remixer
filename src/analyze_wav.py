...
class AudioAnalyzer(object):
    ...
    def getBeats(self):
        beats = self._getBeats()
        for i, beat in enumerate(beats):
            if i == 0:
                continue
            yield self._getNotesForBeat(beats[i - 1], beat)

    def _getBeats(self, params=None):
    ...
    def _getNotesForBeat(self, seconds_start, seconds_end, plotit=False):
